...
fn load_weights_record<B: Backend>(
    ...
    let load_args = LoadArgs::new(torch_weights)
        ...
        .with_key_remap("features\\.0\\.1\\.(.+)", "features.0.norm.$1")
        // Inverted Residual Blocks - Expand Conv
        .with_key_remap(
            "features\\.(1[0-6]|[1-9])\\.block\\.0\\.0\\.(.+)",
            "features.$1.expand_conv.conv.$2",
        )
        .with_key_remap(
            "features\\.(1[0-6]|[1-9])\\.block\\.0\\.1\\.(.+)",
            "features.$1.expand_conv.norm.$2",
        )
        // Inverted Residual Blocks - Depthwise Conv
        .with_key_remap(
            "features\\.(1[0-6]|[1-9])\\.block\\.1\\.0\\.(.+)",
            "features.$1.depthwise_conv.conv.$2",
        )
        .with_key_remap(
            "features\\.(1[0-6]|[1-9])\\.block\\.1\\.1\\.(.+)",
            "features.$1.depthwise_conv.norm.$2",
        )
        ...
        .with_key_remap(
            "features\\.(1[0-6]|[1-9])\\.block\\.3\\.0\\.(.+)",
            "features.$1.project_conv.conv.$2",
        )
        .with_key_remap(
            "features\\.(1[0-6]|[1-9])\\.block\\.3\\.1\\.(.+)",
            "features.$1.project_conv.norm.$2",
        )
        // Last Conv Layer
        .with_key_remap("features\\.16\\.0\\.(.+)", "features.16.conv.$1")
        .with_key_remap("features\\.16\\.1\\.(.+)", "features.16.norm.$1")
        // Classifier
        .with_key_remap("classifier\\.0\\.(.+)", "classifier.0.$1")
        .with_key_remap("classifier\\.3\\.(.+)", "classifier.3.$1")
        .with_debug_print();
...
impl MobileNetV3Config {
    fn create<B: Backend>(
        ...
        let classifier = vec![
            LayerType2D::Linear(
                LinearConfig::new(lastconv_output_channels, last_channel)
                    .init(device),
            ),
            LayerType2D::Activation(Activation::Hardswish(Hardswish::new())),
            LayerType2D::Dropout(DropoutConfig::new(0.2).init()),
            LayerType2D::Linear(
                LinearConfig::new(last_channel, num_classes).init(device),
            ),
        ];
...
impl<B: Backend> MobileNetV3<B> {
    pub fn forward(&self, input: Tensor<B, 4>) -> Tensor<B, 2> {
        ...
        let mut reshaped = x.reshape([batch_size, num_elements]);

        for layer in &self.classifier {
            reshaped = layer.forward(reshaped);
        }
